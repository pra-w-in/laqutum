...
// ─── EXPERIMENTAL FEATURES PATCH ───────────────────────────────────

const originalComputePriority = ConfidenceTracker.computePriorityScore;
const PREREQUISITES = {
    P12: ['P1', 'P3'],
    P11: ['P5'],
    P4: ['P7']
...
let globalChurnTracker = [];
let globalTrapVulnerability = {};

ConfidenceTracker.computePriorityScore = function(pattern) {
    let baseScore = originalComputePriority(pattern);
    const states = ConfidenceTracker.getAllStates();
    
    // Feature 1: Syllabus Coverage Index
    const untouched = Object.keys(states).filter(p => !states[p].lastTouched);
    if (untouched.length > 0) {
        if (!states[pattern].lastTouched) {
            baseScore += 5.0; // Boost unseen patterns
        } else {
            baseScore *= 0.5; // Penalty to already seen patterns if we have unseen ones
        }
    }
    
    // Feature 2: Prerequisite Readiness
    const prereqs = PREREQUISITES[pattern];
    if (prereqs) {
        let allSolid = true;
        for (let req of prereqs) {
            if (states[req].score < 0.65) {
                allSolid = false;
                break;
            }
        }
        if (!allSolid) {
            baseScore *= 0.1; // Massive penalty if prereqs aren't solid
        }
    }
    
    // Feature 4: Trap Vulnerability Profile
    // If the pattern involves traps that the user has historically failed, boost priority
    if (globalTrapVulnerability[pattern] > 0) {
        baseScore += 1.5;
    }
...
const originalGetNextItem = SelectionEngine.getNextItem;
SelectionEngine.getNextItem = function(sessionState) {
    // Feature 3: Cognitive Churn Rate / Circuit Breaker
    // If the last 4 interactions were wrong, override selection with an explanation or walkthrough
    const recentWrong = globalChurnTracker.slice(-4).filter(x => !x).length;
    if (recentWrong >= 3) {
        // Find a pattern to explain
        const queue = SelectionEngine.getPriorityQueue();
        if (queue.length > 0) {
            const p = queue[0].pattern;
            const qList = ContentBank.getQuestions(p);
            if (qList.length > 0 && qList[0].explanation_id) {
                const expl = ContentBank.getById(qList[0].explanation_id);
                if (expl) {
                    globalChurnTracker = []; // Reset circuit breaker
                    return {
                        type: 'explanation',
                        entry: expl,
                        pattern: p,
    ...
    return originalGetNextItem(sessionState);
};
...
function runSingleLearnerSimulation(archetypeKey, archetype, learnerId) {
    ...
    // Learning Effect Tracker
    const patternLearningBoosts = {};
    ...
    while (simulatedTimeSeconds < TWO_DAY_BUDGET_SECONDS) {
        ...
        // If the item is an explanation (circuit breaker hit), boost learning
        if (item.type === 'explanation' || item.type === 'guided_walkthrough') {
            patternLearningBoosts[currentPattern] = (patternLearningBoosts[currentPattern] || 0) + 0.15;
        }

        // Determine answer & comprehension choice
        const effectiveCorrectProb = Math.min(0.95, archetype.correctProb + (patternLearningBoosts[currentPattern] || 0));
        let isCorrect = Math.random() < effectiveCorrectProb;
        ...
        // Track churn and vulnerability
        globalChurnTracker.push(isCorrect);
        if (globalChurnTracker.length > 20) globalChurnTracker.shift();
        
        if (!isCorrect && item.entry && item.entry.traps) {
            globalTrapVulnerability[currentPattern] = (globalTrapVulnerability[currentPattern] || 0) + 1;
        }
...
async function runMonteCarloSimulation() {
    ...
    console.log('\n✅ Experimental Monte Carlo Simulation Completed Successfully!');
    console.log(`📄 Structured Report generated at d:\\aptitude\\experimental_simulation_results.md`);
}

function generateMarkdownReport(report, undefinedStates) {
    let md = `# 📊 EXPERIMENTAL Features Learner Trajectory Simulation Report
...
`;
    ...
    fs.writeFileSync('experimental_simulation_results.md', md, 'utf8');
}
